...
② `HASH JOIN SEMI`는 조인 결과에서 중복 행을 걸러내는 `SORT UNIQUE`·`DISTINCT`와 같은 연산이므로, 적립내역의 중복 가입자ID를 먼저 제거한 뒤 조인한다.

③ 이 플랜은 `NOT EXISTS`가 변환된 `HASH JOIN ANTI`이며, 적립내역에 매칭이 하나도 없는 가입자만 반환한다.
