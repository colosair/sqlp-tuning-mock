...
② BCHR은 (5,000,000 − 200,000) / 5,000,000 = 96.0%로 높아 물리 읽기(Disk 200,000)는 논리 읽기의 4%뿐이고, CPU 40초가 50초의 80%다. 병목은 디스크가 아니라 NL 조인이 `해양심층수취수량`을 200만 번 방문하며 쌓은 논리 읽기 500만이 CPU를 태우는 것이므로, 조인 방식·액세스 경로를 바꿔 논리 읽기 횟수 자체를 줄여야 한다.

③ Elapsed − CPU = 50 − 40 = 10초가 물리 읽기 대기이므로 I/O 바운드다. 병렬도(DOP)를 높여 디스크 읽기를 나눠 주면 50초가 개선된다.
