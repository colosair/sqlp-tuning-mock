...
③ WHERE의 세 조건(관할서코드·점검일시·판정결과)이 모두 `점검_IX`의 인덱스 액세스 조건으로 INDEX RANGE SCAN 단계에서 함께 처리되어 스캔 범위를 결정한다.
...
| ③ | ✗ | 판정결과는 `점검_IX` 구성 칼럼이 아니므로 인덱스 액세스 조건이 될 수 없습니다. 테이블 필터인 판정결과를 인덱스 액세스 조건 집합에 끼워 넣은 진술로, 그것이 스캔 범위를 좁혔다면 INDEX RANGE SCAN Card가 이미 3,600이었을 텐데 실제는 12,000입니다 |
