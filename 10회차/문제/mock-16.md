...
나머지는 어긋납니다. ①은 인터벌을 "생성 시점에 12개월 사전 생성"이라 했지만 인터벌 파티션은 첫 insert 때 필요분만 만들어집니다. ②는 `TO_CHAR(발행일자,'YYYYMM')='202603'`으로 키를 **형변환·가공**해, 3월을 겨냥한 듯 보여도 경계 대조가 불가능해 전 파티션을 스캔합니다 — "3월만 읽는다"는 부분진실입니다. ③은 발행금액이 파티션 키가 아니라 일반 필터라 RANGE 프루닝이 걸리지 않습니다.
