...
② 거래이력은 전체를 모든 서버에 복제(broadcast)하고, 계좌는 재분배 없이 각 서버가 자기 파티션만 스캔한다(broadcast-none).

③ 두 테이블 모두 재분배 없이 QC(질의 조정자)로 먼저 모은 뒤, 단일 서버에서 순차적으로 조인한다.

④ 거래이력은 조인 키 해시로 재분배하고, 계좌는 전체를 모든 서버에 복제(broadcast)한다(hash-broadcast).
...
② broadcast-none : 거래이력 = BROADCAST, 계좌 = 재분배없음  → SEND HASH 두 개와 불일치
④ hash-broadcast : 거래이력 = HASH,      계좌 = BROADCAST   → Id 9가 SEND HASH이므로 불일치
③ QC 단일서버    : 재분배 없음, 단일 서버 조인            → P->P·PX SEND HASH 두 개와 불일치
```
...
| ② | ✗ | broadcast 노드가 없습니다. 거래이력 쪽 Id 5는 BROADCAST가 아니라 `PX SEND HASH`이고, 계좌도 재분배(Id 9)됩니다 |
| ③ | ✗ | 두 생산자 TQ가 `P->P`로 병렬 재분배 중이며 조인(Id 3)도 병렬(PCWP)입니다. QC 취합·단일 서버 조인이 아닙니다 |
| ④ | ✗ | 계좌 쪽 Id 9가 `PX SEND HASH`이므로 broadcast가 아닙니다. 한쪽만 해시라는 hash-broadcast로 값을 뒤바꾼 진술입니다 |
